...
X = daten_dict["state_data.npy"]
input_data = daten_dict["input_data.npy"]

X_plus = X[:, 1:T+1]
X_minus = X[:, 0:T]
U_minus = input_data[:, 0:T]

#X_plus = X[:, 1 + 7*T : 8*T + 1]
#X_minus = X[:, 0 + 7*T : 8*T]
#U_minus = input_data[:, 0 + 7*T :8*T]
...
#Noisemodel, Random uniform noise with maximum amplitude w_max
w_max = 0.01
...
Phi11 = T* w_max**2 * np.eye(X.shape[0]) 
Phi12 = np.zeros((X.shape[0], X_minus.shape[1]))
Phi22 = -np.eye(X_minus.shape[1])
Phi21 = Phi12.T
